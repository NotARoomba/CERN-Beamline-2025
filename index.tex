    ...
    \begin{tikzpicture}
        \node [rectangle, rounded corners=5pt, draw, inner sep=10pt] { % Rounded border
            \begin{tabularx}{0.95\textwidth}{l l X} % Reduced width for border spacing
                ...
                \midrule
                Magnetic Field (\(B\)) & 1-1.35 T         & Controls spacing between Landau levels (LLs). Energy levels scale as \(E_n \propto \sqrt{B|n|}\). At 1.35 T, the \(n=1\) LL is \(\sim\)100 meV above \(n=0\), creating resolvable gaps for DESY's calorimeters. \\
                \midrule
                Beam Energy            & 1-6 GeV          & 1 GeV electrons (\(v \approx 0.94c\)) show weak coupling, while 6 GeV (\(v \approx 0.999c\)) maximizes relativistic interaction. Distinguishes classical vs quantum scattering regimes.                         \\
                \midrule
